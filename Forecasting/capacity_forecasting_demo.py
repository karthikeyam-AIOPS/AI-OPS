...
def generate_and_train_model():
    ...
    gb_hist = np.array(
        [100 + 1.5 * i + np.random.normal(0, 2) for i in range(30)]
    ).reshape(-1, 1)
...
def save_model_and_metadata(model, days_hist, gb_hist, gb_future):
    ...
    model_metadata = {
        ...
        "forecast_summary": {"next_7_days_avg": np.mean(gb_future[:7])},
    }
...
def main():
    ...
    save_model_and_metadata(model, days_hist, gb_hist, gb_future)
